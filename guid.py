...
class LazyFrameworkGUI(QMainWindow):
    def __init__(self):
        ...
        self.browser_stack = None
        self.browser_page = None
        self.browser_page_layout = None
    ...
    def create_left_sidebar(self):
        ...
        # Info Group dengan Browser Controls - DIUBAH
        ...
        browser_tab_layout.setContentsMargins(10, 10, 10, 10)
        browser_tab_layout.setSpacing(15)

        # Tombol buka
        open_btn = QPushButton("Open Browser (New Window)")
        ...
        """)
        open_btn.clicked.connect(self.open_browser_popup)
        browser_tab_layout.addWidget(open_btn)
        ...
        """)
        close_btn.clicked.connect(self.close_browser_popup)
        browser_tab_layout.addWidget(close_btn)

        # Status
        self.browser_status = QLabel("Status: Closed")
        self.browser_status.setStyleSheet("color: #ff5555; font-weight: bold; font-size: 14px;")
        browser_tab_layout.addWidget(self.browser_status)

        browser_tab_layout.addStretch()
        self.info_browser_tabs.addTab(browser_tab, "Browser")

        # Tambahkan tab widget ke info group
    ...
    def open_browser_popup(self):
        if hasattr(self, 'browser_window') and self.browser_window and self.browser_window.isVisible():
            self.browser_window.raise_()
            self.browser_window.activateWindow()
            self.append_output("[dim]Browser already open[/]")
            return

        # === BUAT WINDOW BARU ===
        self.browser_window = QMainWindow()
        self.browser_window.setWindowTitle("LazyFramework Browser")
        self.browser_window.resize(1200, 800)

        # === TOOLBAR ===
        toolbar = self.browser_window.addToolBar("Navigation")
        toolbar.setMovable(False)
        toolbar.setStyleSheet("QToolBar { background: #2d2d2d; }")

        # Back
        back_act = QAction("Back", self)
        back_act.triggered.connect(lambda: self.browser.back() if self.browser else None)
        toolbar.addAction(back_act)

        # Forward
        fwd_act = QAction("Forward", self)
        fwd_act.triggered.connect(lambda: self.browser.forward() if self.browser else None)
        toolbar.addAction(fwd_act)

        # Reload
        reload_act = QAction("Reload", self)
        reload_act.triggered.connect(lambda: self.browser.reload() if self.browser else None)
        toolbar.addAction(reload_act)

        # URL Bar
        self.url_bar = QLineEdit()
        self.url_bar.setPlaceholderText("Enter URL or search...")
        self.url_bar.returnPressed.connect(self.navigate_browser)
        toolbar.addWidget(self.url_bar)

        # === BROWSER ===
        self.browser = QWebEngineView()
        self.browser.urlChanged.connect(lambda u: self.url_bar.setText(u.toString()))
        self.browser.loadStarted.connect(lambda: self.url_bar.setText("Loading..."))
        self.browser.loadFinished.connect(lambda ok: self.url_bar.setPlaceholderText("Enter URL..." if ok else "Failed"))

        self.browser.load(QUrl("https://www.google.com"))

        # Set central widget
        self.browser_window.setCentralWidget(self.browser)
        self.browser_window.show()

        # Update status
        self.browser_status.setText("Status: OPEN")
        self.browser_status.setStyleSheet("color: #50fa7b; font-weight: bold; font-size: 14px;")
        self.append_output("[bold green]Browser opened in new window[/]")
    ...
    def open_browser_panel(self):
        # ... (kode sebelumnya)
        self.browser_status_label.setText("Status: Open")
        self.browser_status_label.setStyleSheet("color: #50fa7b; font-weight: bold;")
    ...
    def close_browser_popup(self):
        if hasattr(self, 'browser_window') and self.browser_window:
            ...
            self.browser_status.setText("Status: Closed")
            self.browser_status.setStyleSheet("color: #ff5555; font-weight: bold; font-size: 14px;")
            self.append_output("[bold red]Browser closed[/]")
        else:
            self.append_output("[dim]No browser to close[/]")
    
    # === BROWSER PANEL METHODS - END ===
    ...
    def open_in_browser(self, url):
        ...
        QTimer.singleShot(500, lambda: self.browser.setUrl(QUrl(url)))
