...
from PyQt6.QtGui import QFont, QTextCursor, QPalette, QColor, QAction, QKeySequence, QIntValidator, QPixmap
...
# === REVERSE TCP MODULE INTEGRATION ===
class ReverseTCPManager(QObject):
    """Manager untuk handle reverse TCP sessions"""
    session_update = pyqtSignal(str, dict)  # session_id, session_data
    
    def __init__(self, framework):
        super().__init__()
        self.framework = framework
        self.reverse_tcp_module = None
        self.load_reverse_tcp_module()
    
    def load_reverse_tcp_module(self):
        """Load reverse_tcp module"""
        try:
            # Coba load module
            from modules.payloads.reverse import reverse_tcp
            self.reverse_tcp_module = reverse_tcp
            print("[+] Reverse TCP module loaded successfully")
        except ImportError as e:
            print(f"[-] Failed to load reverse_tcp module: {e}")
            self.reverse_tcp_module = None
    
    def send_command_to_session(self, session_id, command):
        """Send command to reverse TCP session"""
        if not self.reverse_tcp_module:
            return False, "Reverse TCP module not loaded"
        
        try:
            # Gunakan fungsi dari reverse_tcp module
            success = self.reverse_tcp_module.send_command_to_session(session_id, command)
            return success, "Command sent" if success else "Failed to send command"
        except Exception as e:
            return False, f"Error: {str(e)}"
    
    def get_active_sessions(self):
        """Get active sessions dari reverse_tcp module"""
        if not self.reverse_tcp_module:
            return {}
        
        try:
            return self.reverse_tcp_module.get_active_sessions()
        except:
            return {}
    ...
    def kill_session(self, session_id):
        ...
        try:
            return self.reverse_tcp_module.kill_session(session_id)
        except:
            return False
...
class ModuleRunner(QThread):
    ...
    def run(self):
        try:
            ...
            # **CRITICAL: Setup GUI integration untuk reverse_tcp**
            if "reverse_tcp" in str(self.module_instance).lower():
                self.setup_reverse_tcp_integration()

            # Run the module - coba kedua metode
            if hasattr(self.module_instance, 'run'):
                # Method 1: run(session)
                result = self.module_instance.run(self.framework.session)
            elif hasattr(self.module_instance, 'run_module'):
                # Method 2: run_module(session)  
                result = self.module_instance.run_module(self.framework.session)
            else:
                # Method 3: run() tanpa parameter (fallback)
                result = self.module_instance.run()
                
            if result is False:
                self.output.emit("[red]Module execution failed[/]")
    ...
    def setup_reverse_tcp_integration(self):
        """Setup GUI integration untuk reverse_tcp module"""
        # Setup session management untuk reverse_tcp
        if 'reverse_tcp_sessions' not in self.framework.session:
            self.framework.session['reverse_tcp_sessions'] = {}
        
        # Setup GUI reference untuk reverse_tcp module
        self.framework.session['gui_instance'] = self.framework.gui_instance
        self.framework.session['gui_sessions'] = {
            'dict': self.framework.gui_instance.sessions,
            'lock': threading.Lock()
        }
...
class LazyFrameworkGUI(QMainWindow):
    def __init__(self):
        ...
        # Setup reverse TCP manager
        self.reverse_tcp_manager = ReverseTCPManager(self.framework)
        
        # Replace framework console dengan GUI console
        self.framework.console = GUIConsole(self.append_output)
        self.framework.gui_instance = self  # Reference untuk modules
        ...
        self.session_output_buffer = {}  # Buffer output per session
    ...
    def show_splash_screen(self):
        ...
        for progress, status in loading_steps:
            ...
            time.sleep(1)  # Sedikit lebih cepat dari Burp Suite asli
    ...
    def init_ui(self):
        self.setWindowTitle("LazyFramework GUI v2.6 - Enhanced Session Management")
        ...
        # **NEW: Session auto-update timer**
        self.session_update_timer = QTimer()
        self.session_update_timer.timeout.connect(self.auto_update_sessions)
        self.session_update_timer.start(1000)  # Update setiap 1 detik

    def create_main_content(self):
        """Create main content area dengan enhanced session management"""
        main_widget = QWidget()
        ...
        # **ENHANCED SESSION MANAGEMENT TAB**
        self.session_tab = QWidget()
        self.session_layout = QVBoxLayout(self.session_tab)

        # Header dengan session count
        self.session_header = QLabel("Session Management (0 active)")
        self.session_header.setStyleSheet("font-weight: bold; color: #00ff41; font-size: 16px; margin: 10px;")
        self.session_layout.addWidget(self.session_header)

        # Session List dengan auto-refresh
        session_list_layout = QHBoxLayout()
        ...
        session_actions_layout = QVBoxLayout()
        
        self.refresh_sessions_btn = QPushButton("🔄 Refresh")
        self.refresh_sessions_btn.clicked.connect(self.manual_refresh_sessions)
        self.refresh_sessions_btn.setToolTip("Refresh sessions list")
        session_actions_layout.addWidget(self.refresh_sessions_btn)
        
        self.test_session_btn = QPushButton("🔍 Test Session")
        self.test_session_btn.clicked.connect(self.test_selected_session)
        self.test_session_btn.setToolTip("Test if selected session is alive")
        session_actions_layout.addWidget(self.test_session_btn)
        
        session_actions_layout.addStretch()
        
        session_list_layout.addWidget(self.session_list, 3)
        session_list_layout.addLayout(session_actions_layout, 1)
        
        self.session_layout.addLayout(session_list_layout)

        # Command Input dengan session info
        cmd_layout = QHBoxLayout()
        
        self.session_info_label = QLabel("No session selected")
        self.session_info_label.setStyleSheet("color: #888; font-size: 12px;")
        cmd_layout.addWidget(self.session_info_label)
        
        self.session_cmd_input = QLineEdit()
        self.session_cmd_input.setPlaceholderText("Select a session to send commands...")
        self.session_cmd_input.returnPressed.connect(self.send_session_command)
        cmd_layout.addWidget(self.session_cmd_input, 2)

        send_btn = QPushButton("Send")
        send_btn.clicked.connect(self.send_session_command)
        send_btn.setStyleSheet("background: #0078d4; color: white;")
        ...
        upgrade_btn = QPushButton("⬆️ Upgrade to Meterpreter")
        upgrade_btn.clicked.connect(self.upgrade_session)
        upgrade_btn.setToolTip("Upgrade session to Meterpreter")
        ...
        kill_btn.setToolTip("Kill selected session")
        btn_layout.addWidget(kill_btn)
        
        clear_btn = QPushButton("🗑️ Clear Output")
        clear_btn.clicked.connect(self.clear_session_output)
        clear_btn.setToolTip("Clear session output")
        ...
        # **NEW: Sessions quick button**
        sessions_btn = QPushButton("📡 Sessions")
        sessions_btn.clicked.connect(self.show_sessions_tab)
        sessions_btn.setToolTip("Show sessions management")
        control_layout.addWidget(sessions_btn)
    ...
    def auto_update_sessions(self):
        """Auto-update sessions dari reverse_tcp module"""
        try:
            # Get sessions dari reverse_tcp module
            reverse_sessions = self.reverse_tcp_manager.get_active_sessions()
            
            # Update GUI sessions
            updated = False
            for session_id, session_data in reverse_sessions.items():
                if session_id not in self.sessions:
                    # New session
                    self.sessions[session_id] = session_data
                    updated = True
                    self.append_output(f"[green][+] New session detected: {session_id}[/]")
                else:
            ...
            for session_id in list(self.sessions.keys()):
                if session_id not in reverse_sessions:
            ...
            # Silent error untuk auto-update
            pass

    def manual_refresh_sessions(self):
        """Manual refresh sessions"""
        self.append_output("[yellow][*] Manually refreshing sessions...[/]")
        self.auto_update_sessions()

    def test_selected_session(self):
        """Test if selected session is alive"""
        ...
        if not session:
            self.append_output(f"[red]Session {self.active_session_id} not found[/]")
            return
        
        # Test dengan command 'echo'
        success, message = self.reverse_tcp_manager.send_command_to_session(
            self.active_session_id, "echo 'SESSION_TEST'"
        )
        
        if success:
            self.append_output(f"[green]✓ Session {self.active_session_id} is alive[/]")
        else:
            self.append_output(f"[red]✗ Session {self.active_session_id} is dead: {message}[/]")

    def update_sessions_ui(self):
        """Update sessions UI dengan real data"""
        # Update header
        active_count = len(self.sessions)
        self.session_header.setText(f"Session Management ({active_count} active)")
        
        # Update session list
        self.session_list.clear()
        
        for session_id, session in self.sessions.items():
            item_text = f"{session_id} | {session.get('ip', 'unknown')}:{session.get('port', 'unknown')} | {session.get('type', 'unknown')}"
            ...
            if 'meterpreter' in session_type:
                item.setForeground(QColor('#ff79c6'))  # Pink
            elif 'python' in session_type:
                item.setForeground(QColor('#ffb86c'))  # Orange
            elif 'powershell' in session_type:
                item.setForeground(QColor('#0078d4'))  # Blue
            elif 'bash' in session_type or 'shell' in session_type:
                item.setForeground(QColor('#50fa7b'))  # Green
            else:
                item.setForeground(QColor('#8be9fd'))  # Cyan
        ...
        if self.active_session_id and self.active_session_id in self.sessions:
            ...
            self.session_info_label.setText(
                f"Active: {self.active_session_id} | {session.get('ip', 'unknown')}:{session.get('port', 'unknown')} | {session.get('type', 'unknown')}"
            )
        else:
    ...
    def on_session_selected(self, item):
        ...
        try:
            ...
            if session_id in self.sessions:
                ...
                self.session_output.setPlainText(output_text)
                ...
                self.session_cmd_input.setPlaceholderText(
                    f"Enter command for {session_id} ({session.get('type', 'unknown')})..."
                )
                self.session_cmd_input.setEnabled(True)
            ...
            self.append_output(f"[red]Session selection error: {e}[/]")

    def send_session_command(self):
        """Send command to active session - IMPROVED"""
        if not self.active_session_id:
        ...
        # **PRIORITAS 1: Kirim via Reverse TCP Manager**
        success, message = self.reverse_tcp_manager.send_command_to_session(
            self.active_session_id, command
        )
        
        if success:
            self.append_output(f"[green]✓ Command sent to session {self.active_session_id}[/]")
        else:
            self.append_output(f"[red]✗ Failed to send command: {message}[/]")
            # Fallback ke simulation
            self.simulate_command_response(session, command)

    def simulate_command_response(self, session, command):
        """Fallback command simulation"""
        try:
            ...
            if command_lower in ['whoami', 'id']:
                ...
                response = "/home/victim\n"
            elif command_lower in ['ls', 'dir']:
                response = "file1.txt\nfile2.exe\nsecret.doc\n"
            elif command_lower in ['uname', 'ver']:
                response = "Linux victim-pc 5.15.0-kali8-amd64\n"
    ...
    def kill_session(self):
        ...
        # Kill via reverse_tcp manager
        success = self.reverse_tcp_manager.kill_session(session_id)
        
        if success:
            self.append_output(f"[green][+] Session {session_id} killed[/]")
            ...
            self.append_output(f"[red][!] Failed to kill session {session_id}[/]")

    def clear_session_output(self):
        """Clear session output"""
        if self.active_session_id and self.active_session_id in self.sessions:
            self.sessions[self.active_session_id]['output'] = ""
            self.session_output.clear()
            self.append_output(f"[dim]Cleared output for session {self.active_session_id}[/]")
        else:
            self.append_output("[red]No active session to clear[/]")

    def show_sessions_tab(self):
        """Switch to sessions tab"""
        self.tabs.setCurrentIndex(3)  # Sessions tab
        self.manual_refresh_sessions()
    ...
    def upgrade_session(self):
        ...
        self.append_output(f"[yellow][*] Attempting to upgrade session {self.active_session_id} to Meterpreter...[/]")
        # Ini akan diimplementasikan nanti dengan module meterpreter
        self.append_output("[yellow][*] Meterpreter upgrade feature coming soon...[/]")

    # ... (methods lainnya seperti create_left_sidebar, create_right_sidebar, dll tetap sama)
    ...
    # ... (methods untuk browser, proxy, output handling, dll tetap sama)

    def append_output(self, text):
        """Append output dengan session detection"""
        ...
        session_patterns = [
            ...
            r"Reverse shell connected from (.+?)",
        ]

        # Cek apakah ini session notification
        for pattern in session_patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                session_id = match.group(1)
                self.append_output(f"[bold green][+] Session detected: {session_id}[/]")
                # Trigger session refresh
                QTimer.singleShot(100, self.manual_refresh_sessions)
                break

        # Proses output normal
        raw_text = text
    ...
    def rich_to_html(self, text):
        # ... (implementasi rich_to_html tetap sama)
        import re
        ...
        ansi_to_rich_tag = {
            '0': '[/]', '1': '[bold]', '2': '[dim]', '4': '[underline]',
            '30': '[black]', '31': '[red]', '32': '[green]', '33': '[yellow]',
            '34': '[blue]', '35': '[magenta]', '36': '[cyan]', '37': '[white]',
            '90': '[black]', '91': '[red]', '92': '[bright_green]', 
            '93': '[yellow]', '94': '[blue]', '95': '[magenta]', 
            '96': '[bright_cyan]', '97': '[white]',
        }
    ...
    def format_unicode_table(self, table_text):
        # ... (implementasi format_unicode_table tetap sama)
    ...
    # ... (methods lainnya seperti load_banner, load_all_modules, run_module, dll tetap sama)
    ...
    def load_banner(self):
        try:
            ...
            self.append_output("LazyFramework GUI v2.6 - Enhanced Session Management")
            self.append_output("Type 'help' or click modules to start")
            self.append_output("Auto session detection enabled")
    ...
    def run_module(self):
        ...
        # **CRITICAL: Setup reverse_tcp integration jika module reverse_tcp**
        if "reverse_tcp" in str(self.framework.loaded_module).lower():
...
{proxy_details}
    ...
    # ... (methods untuk browser, proxy, tor rotation dll tetap sama)
    ...
    # ... (methods untuk proxy, tor rotation dll tetap sama)
