...
from PyQt6.QtGui import QFont,QTextCursor, QPalette, QColor, QAction, QKeySequence, QIntValidator, QLinearGradient, QPainter
...
class LazyFrameworkGUI(QMainWindow):
    ...
    def create_new_session(self, match, raw_text):
        """Create new session from detected pattern - IMPROVED VERSION"""
        try:
    ...
    def update_sessions_ui(self):
        """Update sessions UI - IMPROVED VERSION"""
        try:
            # Clear current list
            ...
            # === FIX: Auto-select first session if none selected ===
    ...
    def on_session_selected(self, item):
        """Handle session selection from the list - FIXED VERSION"""
        try:
            if item is None:
                return
            ...
            # === CRITICAL FIX: Set BOTH session IDs ===
                ...
                for i in range(self.session_list.count()):
                    ...
                    if list_item and list_item.data(Qt.ItemDataRole.UserRole) == session_id:
                        ...
                        if list_item:
                            ...
                            # Reset text color based on session type
                            sess_type = self.sessions.get(list_item.data(Qt.ItemDataRole.UserRole), {}).get('type', '')
                            color_map = {
                                "reverse_tcp": "#50fa7b",
                                "meterpreter": "#ff79c6", 
                                "bash": "#8be9fd",
                            ...
                            list_item.setForeground(QColor(color_map.get(sess_type, "#ffffff")))
                
                self.append_output(f"[green]✓ Selected Session {session_id}[/]")
    ...
    def update_session_info(self):
        ...
        # HAPUS LISTENERS COUNT - karena tidak ada active listeners
        # listeners = len(getattr(self.framework, "listeners", []))
        
        sessions_dict = getattr(self.framework, "sessions", {}) or {}
        total_sess = len(sessions_dict)
        online_sess = sum(1 for s in sessions_dict.values() if s.get("alive") or s.get("connected"))
        ...
        # === HTML RED TEAM AGRESIF - TANPA LISTENERS & TARGET OS ===
        html = f"""
        <div style="line-height:1.5;">
            <div style="text-align:center; color:#ff1744; font-size:14pt; letter-spacing:1px; margin-bottom:8px;">
                <b>SESSION CONTROL</b>
            </div>
            ...
            <b style="color:#ff5252;">CURRENT</b>      : <span style="color:#ff5552;">{self.current_module or 'IDLE'}</span><br>
            ...
            <div style="margin-top:10px; font-size:8pt; color:#444; text-align:center;">
                LazyFramework GUI • Red Team Edition
            </div>
        </div>
        """
